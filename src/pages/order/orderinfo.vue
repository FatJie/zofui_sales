        ...
        <van-pull-refresh v-model="isrefresh" @refresh="onRefresh" :head-height="80" success-text="已刷新">
            ...
            <template v-else>
                <div class="order-wrap" v-if="order.taketype == 1 && order.status > 0">
                    <div class="order-detail" v-if="good.address && good.tels && !good.shoplist">
                        <div class="attr">
                            <div class="attr_item" v-if="good.address">
                                <div class="name">领取地址：{{good.address}}</div>
                            </div>
                            <div class="attr_item" v-if="good.tels">
                                <div class="name">{{params.shopbname}}电话：{{good.tels}}</div>
                                <div class="minibtn" @click="callphone" :data-tel="good.tels">拨号</div>
                            </div>
                        </div>
                    </div>
                    <div class="order-detail" v-if="good.shoplist.length > 0" v-for="(item,index) in good.shoplist" :key="index">
                        <div class="attr">
                            ...
                            <div class="attr_item" v-if="item.tels">
                                <div class="name">{{params.shopbname}}电话：{{item.tels}}</div>
                                <div class="minibtn" @click="callphone" :data-tel="item.tels">拨号</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="order-wrap" v-if="shop.tels && order.taketype == 2 && order.status > 0">
                    <div class="order-detail">
                        <div class="attr">
                            <div class="attr_item" v-if="shop.tels">
                                <div class="name">{{params.shopbname}}电话：{{shop.tels}}</div>
                                <div class="minibtn" @click="callphone" :data-tel="shop.tels">拨号</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <template v-if="order.isshowqr == 1">
                <div v-if="shop.uid!=1690 || shop.uid!=16227">
                ...
                <div class="goodsHx-list goodsHX-common" v-if="order.status > 0  && order.status != 5">
                    <div class="btm goodscode-btm">
                        <div class="list aluse" >
                            ...
                            <div class="info" v-for="(item,index) in order.othercode" :key="index">
                                <div class="code">
                                    领取编码：{{item.code}}
            ...
            <van-steps direction="vertical" :active="0" v-if="express.length > 0">
                <van-step v-for="(item,index) in express" :key="index">
                    <div>{{item.time}}</div>
                    <p>{{item.desc}}</p>
                </van-step>
            </van-steps>
            ...
            <template v-if="order.gtype == 7">
                <div class="group_box bindorder_box item_cell_box" v-for="(item,index) in bindorder" :key="index" @click="link" :data-url="item.appurl">
            ...
            <div class="order-wrap mt20" v-if="order.gtype != 7 && hxlog.length > 0">
                <div class="order-detail">
                    <div class="attr">
                        <div class="attr_item" v-for="(item,index) in hxlog" :key="index">
                            <div class="name">
                                {{item.time}}，使用一次 
	...
	export default {
		name : 'orderinfo',
	  	...
	  	created:function(e){
            ...
            if(this.$route.query.status && this.$route.query.status=="待使用"){
                this.checkGroup()
            }
	  	...
	  	methods: {
            ...
            checkGroup(){
                ...
                timer = setInterval(function(){
                    self.$util.http('good','GET',{orderid:self.order.orderid,op:'orderstatus'},function(res){ 
                        if (res.errno==0) {
                            self.$router.push('/commentorder?id='+self.order.orderid);
                            clearInterval(timer);
                        }
