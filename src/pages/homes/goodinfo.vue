...
<div :class="params.style" id="main">
        ...
        <van-pull-refresh v-model="isrefresh" @refresh="onRefresh" :head-height="80" success-font="已刷新">

            <van-swipe class="info-swiper goods-swiper" v-if="good.img && good.img.length > 0" :autoplay="autoplay" :indicator-color="maincolor" :height="340">
                ...
                <van-swipe-item v-for="(item,index) in good.img" :key="index" @touchstart="stopswiper">
                    <img :src="item" style="width: 100%;height:340px" @load="loadswip" />
                </van-swipe-item>
            </van-swipe>
                ...
                <div class="Lotterymodule ">
                    ...
                    <div class="Luckycode" v-if="lottery && lottery.list.length >= 0 && lottery.status != 0">
                        ...
                        <div catchtap="lottery.list.length>9?'lookmore':'nomore'" class="LCcon" :class="lottery.list.length==1?'LCcon2':''">
                            <div class="LCconma" :class="lottery.list.length==1?'LCconma2':''" v-if="index<9" v-for="(item,index) in lottery.list" :key="index">{{item.code}}</div>
                            <div class="LCconma" v-if="index >=9 && reservations" v-for="(item,index) in lottery.list" :key="index">{{item.code}}</div>
                        </div>
            ...
            <div v-else-if="good.type == 6" class="info-section noborder-top">
                <div class="info-top">
                    <goodbar :good="good" @countdownfn="countdownfn" />
                    <div class="o-top">
                        ...
                        <div class="top-rt" style="float:right;margin-top: 5px;" v-if="params.showscan == 0 || params.iscollect == 0">
                            <div class="top_func" style="float:left;" v-if="params.showscan == 0">
                                <div class="iconfont"></div>
                                <div>{{good.scan}}</div>
                            </div>
                            <div @click="bindCollect" style="float:right;" class="top_func" :class="{'color-pri':good.iscollect}" v-if="params.iscollect == 0">
                                <div class="iconfont">
                                    {{good.iscollect ? '':''}}
                                </div>
                    ...
                    <form v-if="params.isucard != 1 && (good.disprice > 0 || good.isuser == 1)">
                        ...
                        <div class="btn-formid" v-if="good.isuser == 0">
                            ...
                            <div class="o-vip" :class="params.gtstyle == 0 ? 'gtstyle0' : 'gtstyle1'" @click="link" data-url="/buycard?miss=666" v-else>
                                <div class="vip_name">
                                    <font class="iconfont"></font>{{good.isuser == 1 ? params.cardname + (params.twoucard == 1 ? ','+params.upcname : '') : params.upcname}}
                                </div>
                                <div class="vip-ct">
                        ...
                        <div class="btn-formid" v-else>
                            <div class="o-vip" @click="link" data-url="/buycard?miss=666">
                                <div class="vip_name">
                                    <font class="iconfont"></font>{{good.isuser == 1 ? params.cardname + (params.twoucard == 1 ? ','+params.upcname : '') : params.upcname}}
                                </div>
                                <div class="vip-ct">
            ...
            <div class="info-section noborder-top" v-else>
                <div class="info-top">
                    ...
                    <div class="price">
                        <!-- 特权购买 -->
                        <div v-if="good.type == 8">
                            <div class="price_now color-pri">
                                <div>
                                    ¥<font class="num">{{good.oldprice}}</font>{{good.qi}}
                                </div>
                                
                            </div>
                            <div style="float:right;margin-top: 5px;" class="top-rt" v-if="(params.showscan == 0 || params.iscollect == 0) && params.gtstyle == 0">
                                <div style="float:left;" class="top_func" v-if="params.showscan == 0">
                                    <div class="iconfont"></div>
                                    <div>{{good.scan}}</div>
                                </div>
                                <div style="float:right;" @click="bindCollect" class="top_func" :class="{'color-pri':good.iscollect}" v-if="params.iscollect == 0">
                                    <div class="iconfont">
                                        {{good.iscollect ? '':''}}
                                    </div>
                                    <div>{{good.iscollect ? '已收藏':'收藏'}}</div>
                    ...
                    <div style="width:100%;font-size: 0.36rem;display: flex;margin-bottom: 0.2rem;flex-wrap: wrap;">
                        <div style="padding:1px 3px;border-radius: 3px; background: rgba(251, 45, 55,.2);color: #fb2d37;margin-right: 0.1rem;margin-bottom: 0.1rem;" v-for="(item,index) in good.know" :key="index">{{item}}</div>
                    </div>
                    ...
                    <div v-if="good.grade==0 && good.usercardprice!=0">
                        <div class="o-vips" @click="link" data-url="/buycard?miss=666" >
                            <div class="vip_names">
                                <font class="iconfonts">VIP</font>省
                            </div>
                            <div class="vip-cts" v-if="good.type != 8">
                                {{good.addmoney}}元!
                            </div>
                            <div class="vip-cts" v-if="good.type == 8">
                                {{good.oldprice}}元!
            ...
            <div class="promise_authen" v-if="good.promise || good.authen">
                <div v-if="good.promise" style="display:flex;">
                    <div style="padding:2px 4px;border-radius: 3px; background: rgba(251, 45, 55,.1);color: #fb2d37;margin-right: 0.2rem;margin-bottom: 0.1rem;font-size: 0.35rem;">保障店铺</div>
                    <div style="font-size: 0.4rem;color: #444;font-weight: 600;">该店铺已缴纳保证金</div>
                </div>
                <div v-if="good.authen" style="padding-top:0.2rem;display:flex;">
                    <div style="padding:2px 4px;border-radius: 3px; background: rgba(251, 45, 55,.1);color: #fb2d37;margin-right: 0.2rem;margin-bottom: 0.1rem;font-size: 0.35rem;">认证店铺</div>
                    <div style="font-size: 0.4rem;color: #444;font-weight: 600;">该店铺已实名认证</div>
                </div>
            </div>
            ...
            <div style="padding:0.4rem;" v-if="goodlist.length!=0">
                <div style="font-size: .4rem;font-weight: bold;color: rgb(51, 51, 51);padding-bottom:0.2rem;">更多套餐</div>
                <div v-for="(item,index) in goodlist" :key="index">
                    <div @click="toGoodinfo(item.id)" style="width:100%;display:flex;align-items:center;border-bottom:1px solid #eee;padding:0.4rem 0;">
                        <div style="font-size: .3rem;color: #fff;background:#ff2726;border-radius:4px;padding:2px;margin-right:5px;">
                            <font v-if="item.type==1">抢</font>
                            <font v-if="item.type==8">特</font>
                            <font v-if="item.type==2">拼</font>
                            <font v-if="item.type==0">精</font>
                            <font v-if="item.type==9">礼</font>
                        </div>
                        <div style="font-size: .38rem;color: rgb(51, 51, 51);width:50%;height:0.96rem;overflow:hidden;">{{item.title}}</div>
                        <div style="display:flex;font-size:0.36rem;margin-left:0.3rem;">
                            <div>
                                ￥<font style="font-size:0.45rem;font-weight:700;">{{item.show_price ? item.show_price :0}}</font>
                            </div>
                            <div style="margin-left:5px;color:#ff2726;" v-if="item.type!=8">
                                赚￥<font style="font-size:0.45rem;font-weight:700;">{{item.earn_price ? item.earn_price : 0}}</font>
                            </div>
                        </div>
                    </div>
                </div>
                <div style="width:100%;font-size:0.35rem;font-weight:600;color:#ff2726;text-align:center;margin-top:0.3rem;" v-if="goodlists.length>2" @click="closeMore">
                    {{ show_more ? '查看更多' : '收起'}}
                </div>
            </div>
            ...
            <div class="info-section" v-if="shop">
                <!-- <div class="info-title bottitle">{{params.shopbname}}详情<font class="bott bg-pri" :style="params.hengxx == 1 ? 'width:65px' : ''"></font></div> -->
                <div class="info-shop">
                    <div class="shop-item" v-if="shop" >
                        <div style="font-weight:bold;color:#333;margin-top:5px;" class="namebox item_cell_box" @click="link" :data-url="'/shopinfo?id='+shop.id" >
                            <div class="item_cell_flex">{{shop.name}}</div>进入店铺
                            <div class="van-icon van-icon-arrow"></div>
                        </div>
                        ...
                        <div class="row" v-if="shop.tels" @click="bindtap($event,'tel')" :data-tel="shop.tels">
                            <div class="label">· {{params.shopbname}}电话：</div>
                            <div class="info">{{shop.tels}}</div>
                            <div class="btn lbg-pri">拨打</div>
                        </div>
                        <div class="row" v-if="shop.address"  @click="bindtap($event,'loc')":data-lat="shop.lat/1000000" :data-lng="shop.lng/1000000"  :data-add="shop.address" >
                            <div class="label">· {{params.shopbname}}地址：</div>
                            <div class="info">{{shop.address}}</div>
                            <div class="btn lbg-pri">导航</div>
                        </div>
                    </div>
                </div>

                <div class="info-section" v-if="good.contents!=''">
                    <!-- <div class="info-title bottitle">购买须知<font class="bott bg-pri" ></font></div> -->
                    <div class="info-shop" style="width:100%;font-size: 0.4rem;">
                        <div v-html="good.contents"></div>
                    </div>
                </div>
                ...
                <div class="info-shop" v-for="(item,index) in fendian" :key="index" v-if="showallfd || index == 0">
                    <div class="shop-item" >
                        ...
                        <div style="font-weight:bold;color: #333;margin-top:5px;" class="namebox item_cell_box" @click="link" :data-url="'/shopinfo?id='+shop.id" >
                            <div class="item_cell_flex">{{item.name}}</div>进入店铺
                            <div class="van-icon van-icon-arrow"></div>
                        </div>
                        <div class="row" v-if="item.tel"  @click="bindtap($event,'tel')" :data-tel="item.tels">
                            <div class="label">· {{params.shopbname}}电话：</div>
                            <div class="info">{{item.tels}}</div>
                            <div class="btn lbg-pri">拨打</div>
                        </div>
                        <div class="row" v-if="item.address" @click="bindtap($event,'loc')" :data-lat="item.lat/1000000" :data-lng="item.lng/1000000" :data-addname="item.name" :data-add="item.address">
                            <div class="label">· {{params.shopbname}}地址：</div>
            ...
            <div class="info-section">
                ...
                <div class="info-detail">
                    <div class="info-tab thin-border_b">
                        <div  class="item active border-pri">
                            <font>商品评价</font>
                        </div>
                    </div>
                            ...
                            <div class="info-comment" v-if="comment">
                                <div class="item thin-border_b" v-for="(item,index) in comment" :key="index">
                                    ...
                                    <div class="rt">
                                        ...
                                        <div class="rt_star">
                                            <van-rate size="15px" v-model="item.star*1" disabled-color="#FFD21E" disabled />
                                        </div>
        ...
        <!-- <div class="pop" v-if="showpop && good.grade==0">
            <img src="../../assets/images/err.png" @click="showpop=false">
            <img src="http://ykd.lsh199.com/attachment/images/global/mobile/money199.png" @click="link" data-url="/buycard">
        </div> -->
        ...
        <div class="fix-box">
            <div class="info-menu">
                <div class="menu-items">
                    ...
                    <div @click="openShareArea" class="item">
                        <div v-if="good.type==8 || good.grade==0">
                ...
                <div v-if="good.type != 3 && good.type != 8 " class="menu-btns">
                ...
                <div v-else-if="good.type == 8" class="menu-btns">
                    <!-- <template v-if="good.grade!=0"> -->
                        <div class="btn lbg-pri" v-if="good.over_type==0" @click="getSpecial(good.id)">
                            {{good.button}}
                        </div>
                        <div class="btn btn-gray" v-else-if="good.over_type==1">
                            {{good.button}}
                        </div>
                        <div class="btn btn-gray" v-else>
                            {{good.button}}
                        </div>
                    <!-- </template> -->
                    <!-- <template v-else>
                        <div style="width:96%;height: 1.06667rem;margin:0 auto;font-size:0.42rem;color: #fff;display: flex;margin-top: 0.10667rem;text-align: center;line-height: 1.06667rem;font-weight: 700;">
                            <div style="width: 50%;background: #19ec27;border-top-left-radius: 1.33333rem;border-bottom-left-radius: 1.33333rem;" @click="buyit">立即购买</div>
                            <div style="width: 50%;background: #FC2B36;border-top-right-radius: 1.33333rem;border-bottom-right-radius: 1.33333rem;" @click="getSpecial(good.id)">免费领取</div>
                        </div>
                    </template> -->
                </div>
            ...
            <div @click="openShareArea" class="fenxiao-share" v-if="good.show_fx > 0.01 && params.ptopf != 1">
                <img src="http://img11.360buyimg.com/ddimg/jfs/t1/44619/6/15562/1496/5dbef268Ebdc762f2/d1fc338c8c4e187e.png">约赚{{good.money}}元
            </div>
        ...
        methods: {
            ...
            closeMore(){
                ...
                if(this.goodlist.length==2){
                    ...
                    this.goodlist=this.goodlists.slice(0,2);
                    this.show_more=true;
                }
            ...
            getSpecial(id){
                if(this.userinfo.parent==0 || !this.userinfo.parent){
                    ...
                    this.$util.http('good','GET',{op:'goodsbutton',goodid:id},res=>{
                        if(res.data.list.err==0){
                            this.$router.push({path:'/specialdetail',query:{id:id}});
                        }else if(res.data.list.err==1){
                            this.isforvermember=res.data.list.isforevermember;
                            Dialog.confirm({
                                title: '温馨提示',
                                message: res.data.list.mess,
                                confirmButtonText:'马上升级',
                                confirmButtonColor:'#ff6e40',
                                cancelButtonText:'下次再说',
                                cancelButtonColor:'#999'
                            }).then(() => {
                                if(this.isforvermember==1){
                                    this.$router.push('/buycard?isdetail=gooddetail&isforvermember=1');
                                }else{
                                    this.$router.push('/buycard?miss=666&isdetail=gooddetail');
                                }
                                
                            }).catch(() => {
                                
                            });
                        }else if(res.data.list.err==2){
                            Dialog.confirm({
                                title: '温馨提示',
                                message: res.data.list.mess,
                                confirmButtonText:'马上升级',
            ...
            buyit(e){   
                if(this.userinfo.parent==0 || !this.userinfo.parent){
                    ...
                    //是否绑定手机号
                    // if(self.userinfo.account=='' || self.userinfo.account==null){
            ...
            initData(gid){
                ...
                self.$util.http('good','GET',postdata,function(res){
                    ...
                    if( res && !res.errno ){     
                        ...
                        if(res.data.goodlist.length>2){
                            self.goodlist = res.data.goodlist.slice(0,2)
                            self.goodlists = res.data.goodlist
