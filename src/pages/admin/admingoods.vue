                    ...
                    <template v-if="datalist.length > 0">
                        ...
                        <div v-for="item in datalist" :key="item.id" class="goodlist_item" >
                            ...
                            <div class="list_bot item_cell_box">
                                ...
                                <div class="list_btn" v-if="item.status==1" @click="isShelves" :data-id="item.id">上架</div>
                                <div class="list_btn" v-if="item.status==0" @click="isShelves" :data-id="item.id">下架</div>
                                <div class="list_btn" @click="link" :data-url="item.appurl">详情</div>
                                <div class="list_btn" v-if="params.iscpub != 1" @click="edit" :data-type='item.type' :data-id='item.id'>编辑</div>
	...
	export default {
	  	...
	  	data : function(){
	  		return {
                ...
                tab : ['全部','精选','限抢','拼团','砍价','报名','抽奖','秒杀','特权'],
                actindex : 1,
                showexpress : false,
                showtaked : false,
                expressname : '',
        ...
        activated:function(e){
	  	...
	  	methods: {
            ...
            //商品上下架
            isShelves(e){
                this.$util.http('admin','GET',{op:'editgoodsstatus',id:e.currentTarget.dataset.id},res=>{
                    if(res.errno==0){
                        this.$toast({
                            message:res.message,
                            onClose:()=>{
                                this.$util.http('admin','GET',{op:'goodslist',page:this.page},res=>{
            ...
            edit(e){
                ...
                if(e.currentTarget.dataset.type!=8){
                    this.$router.push({path:'/pubgood',query:{id:e.currentTarget.dataset.id}})
                }else{
                    this.$router.push({path:'/pubspecial',query:{id:e.currentTarget.dataset.id}})
                };
            },
            onRefresh(){
                this.initData();
            ...
            initData(){
	  		...
	  		onLoad() {
                var self = this;
	            self.$util.pageList(this,self.page,function(res){
