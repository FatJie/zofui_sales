<template>
  	<div id="app">
        ...
        <transition :name="transitionName" >
            <keep-alive  exclude="index,memberbag,buycard,topay,extension2,referrals,addcard,shopverify,cashdetails,onlinepay,onlinepaying,extension,orderinfo,specialdetail,poster,speciallist,finelist,profitshop,shopdetails,editstaff,chooseshop,adminmine,adminindex,mymoney,rundata,mymembers,suretixian,pubspecial,freeshop,pubgood,ucard,pubgood,user,login,cardmyinfo,helpinfo,groupinfo,commentinfo,shopinfo,redbaginfo,jiziinfo,buycard,orderlist,commentorder,adminbindadmin,zhuanpan,agentjoin,adminpub,adminlogin,cart,adminbaddcard,adminset,adminpay,adminindex,adminhexiaocard,adminhexiao,cardinfo,agentindex,fenxiaouser,fenxiaojoinPartner,orderaddaddress,kefuindex,kefulist,userset,page,shoplist,fenxiaogoods,fenxiaoindex,vedioindex,art,questinfo,adminorderinfo,cardredbag,goodsshare,questindex,adminorder">
...
<script>
    ...
    export default {
        ...
        updated() {
            var unScrollPages=['sortindex'];
            if(unScrollPages.includes(this.$route.name)){
                return;
            }
            window.scroll(0, 0);
        },
